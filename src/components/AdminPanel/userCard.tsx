...
<DropdownMenu>
  ...
  {session?.user?.profileImage ? (
    ...
    <span className="h-8 w-8 flex justify-center items-center rounded-full bg-primary-light text-primary font-semibold">
  ...
  <span>{session?.user?.name ?? ''}</span>
          ...
          <DropdownMenuItem asChild>
            ...
            <DropdownMenuItem asChild>
              <Link href="/dashboard" className="w-full cursor-pointer hover:bg-neutral-200 dark:hover:bg-neutral-700 flex items-center text-sm text-neutral-800 dark:text-neutral-200">
