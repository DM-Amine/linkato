...
export function LinksManager({ links, onLinksUpdate }: LinksManagerProps) {
  ...
  const updateLink = (id: string, title: string, url: string) => {
    ...
    onLinksUpdate(links.map((link) =>
      link.id === id ? { ...link, title, url: formattedUrl } : link
    ))
    setEditingLink(null)
    setEditingValues({ title: "", url: "" })
  }
          ...
          <Card className="border-dashed shadow-none border-neutral-500 bg-neutral-50 dark:bg-neutral-900 py-2">
            <CardContent className="pt-4 px-2 space-y-3 text-neutral-700 dark:text-neutral-300">
              ...
              <div className="flex gap-2">
                <Button onClick={addLink} variant={"default"} className="px-3">
                  Add
                </Button>
                <Button
          ...
          <Droppable droppableId="links">
            {(provided) => (
              <div {...provided.droppableProps} ref={provided.innerRef} className="space-y-2">
                {links
                  .filter((link) => link && link.id && link.title) // avoid rendering empty
                  .map((link, index) => (
                          ...
                          {editingLink === link.id ? (
                            <div className="flex-1 space-y-2">
                              <Input
                                ...
                                onKeyDown={(e) => {
                                  if (e.key === "Enter") updateLink(link.id, editingValues.title, editingValues.url)
                              ...
                              <Input
                                ...
                                onChange={(e) =>
                                  setEditingValues({ ...editingValues, url: e.target.value })
                                }
                                onKeyDown={(e) => {
                                  if (e.key === "Enter") updateLink(link.id, editingValues.title, editingValues.url)
                                  if (e.key === "Escape") setEditingLink(null)
                              ...
                              <div className="flex gap-2 pt-1">
                                <Button onClick={() => updateLink(link.id, editingValues.title, editingValues.url)} className="px-3">
                                  Save
                                </Button>
                                <Button
