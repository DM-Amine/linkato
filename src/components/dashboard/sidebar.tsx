  ...
  return (
    <aside className={`sidebar bg-neutral-200 dark:bg-neutral-900 ${collapsed ? "w-9" : "w-48"} h-screen flex flex-col border-r border-neutral-400 dark:border-neutral-600 transition-all duration-300`}>
      ...
      <div className="sidebar-content w-full flex flex-col items-start justify-start px-1 py-4 gap-2">
        ...
        {/* Pages group with optional child */}
        <div className={`w-full flex flex-col gap-1 ${isPagesSection ? activeStyles : ""} p-1 rounded-md`}>
          {/* Pages Link */}
          <Link
            href="/dashboard/pages"
            className={`sidebar-link w-fit sm:w-full font-normal text-sm transition duration-300 rounded-md hover:bg-primary/60 dark:hover:bg-d-primary-light/40 px-1 py-0.5 flex items-center ${collapsed ? "justify-center" : "gap-2"} text-neutral-700 dark:text-neutral-200`}
          >
          ...
          {/* Nested pageSlug link */}
          {pageSlug && (
            <Link
              href={`/dashboard/pages/${pageSlug}`}
              className={`sidebar-sublink max-w-full truncate font-normal text-sm transition duration-300 rounded-md px-1 py-0.5 flex items-center gap-2 ${
                pathname.startsWith(`/dashboard/pages/${pageSlug}`) ? "text-neutral-700 dark:text-neutral-200 bg-primary/50 hover:bg-primary/80 dark:bg-neutral-900 hover:dark:bg-neutral-900/60 font-semibold" : "text-neutral-600 dark:text-neutral-300"
              }`}
            >
           <CornerDownRight className={`${collapsed ? "" : "ml-2"} shrink-0`} size={16} />
        ...
        {/* Show Analytics only if pageSlug is available */}
        {pageSlug && (
          <Link
            key="Analytics"
            ...
            className={`sidebar-link w-fit sm:w-full font-normal text-neutral-700 dark:text-neutral-200 hover:bg-primary-light/40 dark:hover:bg-d-primary-light/40 text-sm transition duration-300 p-1 rounded-md ${
              pathname === `/dashboard/pages/${pageSlug}/analytics` ? activeStyles : ""
            }`}
          >
        ...
        <Link
          ...
          className={`sidebar-link w-fit sm:w-full font-normal text-neutral-700 dark:text-neutral-200 hover:bg-primary-light/40 dark:hover:bg-d-primary-light/40 text-sm transition duration-300 p-1 rounded-md ${
            pathname === "/dashboard/settings" ? activeStyles : ""
          }`}
        >
          <div className={`flex items-center ${collapsed ? "justify-center" : "gap-2"}`}>
            <span className="text-neutral-600 dark:text-neutral-400">
